...
- **Replay:** presenting an already-rotated token is treated as theft → entire family revoked, audited (`identity.session.replay_detected`, `identity.session.family_revoked`) — **except** within the **concurrency grace window**: reuse ≤ 15 s after rotation (`spexcrafters.auth.refresh-grace`) fails with 401 **without revoking the family** (the shared session cookie already carries the winner's tokens); **only post-grace reuse revokes**. The grace 401 uses the distinct problem type `…/problems/concurrent-refresh` (vs `…/problems/authentication-failed`) so the BFF can keep the session alive on a benign race instead of signing the user out. Raw successor tokens are never persisted (hash-only), so the winner's pair cannot be re-issued to the loser — but it doesn't need to be: the losing tab's next request goes through the BFF, which re-reads the rotated `sc_session` cookie set by the winning response. Rationale: multi-tab browsers race benignly on refresh; the grace window preserves strict rotation while not logging users out for opening two tabs. After the window, reuse is always treated as hostile.
- Concurrent-refresh behavior is covered by dedicated Testcontainers tests (parallel same-token refresh → exactly one rotation, losers 401 with the family left alive; post-window reuse → family revocation).
