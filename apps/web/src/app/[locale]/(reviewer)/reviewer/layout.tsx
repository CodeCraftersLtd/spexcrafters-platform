...
export default async function ReviewerLayout({
  ...
  if (!session) {
    redirect(
      `/${locale}/auth/login?returnTo=${encodeURIComponent(`/${locale}/reviewer`)}`,
    );
  }

  const [common, nav, navigation, a11y] = await Promise.all([
    getTranslations({ locale, namespace: 'common' }),
    getTranslations({ locale, namespace: 'reviewer.nav' }),
      ...
      <aside className={styles.sidebar}>
        ...
        <nav aria-label={nav('label')}>
          <ul className={styles.navList}>
            <li>
              <Link className={styles.navLink} href={`/${locale}/reviewer`}>
                {nav('queue')}
              </Link>
            </li>
          </ul>
